...
namespace FormTest
{
    public class UserApi
    {
        ...
        private void KeyboardComm(byte[] combo)
        {
            if (combo.Length >= 2)
            {
                //all keys down
                ...
                {
                    virtual_KeyboardEvent(__bVk, KeyEventFlag.Down);
                }
                //Reverse keys
                ...
                {
                    virtual_KeyboardEvent(key, KeyEventFlag.Up);
                }
            }
        }

        private byte getKeys(Keys key)
        {
            return (byte)key;
        }
